...
class AtlasBuildTest {

    BasicMove basicmove;

    AtlasBuild atlasbuild;
    GameBoard gameBoard;
    TurnInfo turnInfo;
    Player player;
    String piece;
    ...
    void checkBuild (){
        ...
        assertEquals(GameMessage.hasNotMoved, atlasbuild.checkBuild(turnInfo, gameBoard,player,0,buildingTo, piece));
        ...
        assertEquals(GameMessage.alreadyBuilt, atlasbuild.checkBuild(turnInfo, gameBoard,player,0,buildingTo, piece));
        turnInfo.turnInfoReset();
        ...
        assertEquals(GameMessage.notSameThatMoved, atlasbuild.checkBuild(turnInfo, gameBoard,player,0,buildingTo, piece));
        turnInfo.setChosenWorker(0);
        ...
        assertEquals(GameMessage.notOwnPosition, atlasbuild.checkBuild(turnInfo, gameBoard,player,0,buildingTo, piece));

        //workerPosition must be adjacent to building position
        buildingTo[0]=2;
        buildingTo[1]=0;
        assertEquals(GameMessage.notInSurroundings, atlasbuild.checkBuild(turnInfo, gameBoard,player,0,buildingTo, piece));
        ...
        assertEquals(GameMessage.noBuildToCompleteTower, atlasbuild.checkBuild(turnInfo, gameBoard,player,0,buildingTo, piece));
        ...
        buildingTo[0]=3;
        buildingTo[1]=2;
        assertEquals(GameMessage.noBlocksInDome, atlasbuild.checkBuild(turnInfo, gameBoard,player,0,buildingTo, piece));
        ...
        buildingTo[0]=2;
        buildingTo[1]=4;
        assertEquals(GameMessage.noBuildToOccupiedTower, atlasbuild.checkBuild(turnInfo, gameBoard,player,0,buildingTo, piece));
        ...
        buildingTo[1]=3;
        //build ok
        assertEquals(GameMessage.buildOK, atlasbuild.checkBuild(turnInfo, gameBoard,player,0,buildingTo, piece));

    }


    @Test
    void atlasBuildTesting(){

        //Dome in a level 2 test
        turnInfo.setChosenWorker(0);
        piece = "Dome";
        player.getWorker(0).movedToPosition(2,2,0);
        turnInfo.setHasMoved();
        ...
        buildingTo[1]=3;
        assertEquals(GameMessage.buildOK, atlasbuild.checkBuild(turnInfo, gameBoard,player,0,buildingTo, piece));
        assertEquals(GameMessage.turnCompleted, atlasbuild.build(turnInfo, gameBoard,player,0,buildingTo, piece));
